 ...
 * @param color_pair The color pair to use for the window background (default is
 * 0, which means no color).
 */
class Window {
public:
...
private:
  ...
  std::vector<std::unique_ptr<Window>>
      subwindows; // Subwindows within this window
...
public:
  ...
  std::reference_wrapper<Text> setTitle(const std::string &text,
                                        Alignment alignment = Alignment::LEFT,
                                        int color_pair = 0, int offset = 0);
