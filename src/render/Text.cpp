...
size_t Text::getLength() const {
  return std::visit(
      [](const auto &chunks) {
        auto view = chunks | std::views::transform([](const auto &chunk) {
                      return chunk.text.size();
                    });
        return std::accumulate(view.begin(), view.end(),
...
bool Text::isEmpty() const {
  return std::visit([](const auto &chunks) { return chunks.size() <= 0; },
                    textChunks);
}
...
std::variant<std::string, std::wstring> Text::getText() {
  return std::visit(
      [](const auto &chunks) -> std::variant<std::string, std::wstring> {
