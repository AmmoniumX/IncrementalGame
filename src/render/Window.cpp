...
std::reference_wrapper<Text> Window::setTitle(const std::string &text,
                                              Alignment alignment,
                                              int color_pair, int offset) {
  if (title) {
...
Window::Window(int x, int y, int width, int height, bool visible,
               int color_pair, WINDOW *parentWin)
    : win(newWin(height, width, y, x)), x(x), y(y), width(width),
      height(height), visible(visible), color_pair(color_pair),
      parentWin(parentWin) {
  if (color_pair > 0) {
...
Window::Window(Window &&other) noexcept
    : win(std::move(other.win)), x(other.x), y(other.y), width(other.width),
      height(other.height), visible(other.visible),
      color_pair(other.color_pair), parentWin(other.parentWin) {
  if (color_pair > 0) {
    wbkgd(win.get(), COLOR_PAIR(color_pair));
  }
}
...
void Window::enable() { visible = true; }
void Window::disable() {
  clearWindow();
  visible = false;
}
...
Window &Window::createSubwindow(int subY, int subX, int subWidth, int subHeight,
  ...
  subwindows.push_back(std::make_unique<Window>(
      subX, subY, subWidth, subHeight, visible, color_pair, win.get()));
